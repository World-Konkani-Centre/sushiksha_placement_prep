...
from resume_builder.forms import ContactModelForm,AboutModelForm,SkillModelForm,EducationModelForm,InternshipFormExperienceForm,TrainingCertificationForm,ProjectForm,ExtraModelForm,LanguageModelForm,PIModelForm,AchievementModelForm,DeclarationModelForm,OtherModelForm
from resume_builder.models import Contact,About,Skill,Education,Internship_Experience,Training_Certification,Project,Extra,Language,Personal_Interest,Achievement,Declaration,Other
...
def internship(request):
    ie = Internship_Experience.objects.filter(user=request.user)
    if request.method == 'GET':
...
def internship_edit(request,id):
    internship = Internship_Experience.objects.get(user=request.user,id=id)
    if request.method=='GET':
...
def internship_delete(request,id):
    intern = Internship_Experience.objects.get(id=id,user=request.user).delete()
    return redirect('resume-internship')
...
def training(request):
    train = Training_Certification.objects.filter(user=request.user)
    if request.method == 'GET':
...
def training_edit(request,id):
    train = Training_Certification.objects.get(user=request.user,id=id)
    if request.method=='GET':
...
def training_delete(request,id):
    intern = Training_Certification.objects.get(id=id,user=request.user).delete()
    return redirect('resume-training')
...
def pi(request):
    try:
        pi = Personal_Interest.objects.get(user=request.user)
    except Personal_Interest.DoesNotExist:
        pi = None
    if request.method == 'GET':
